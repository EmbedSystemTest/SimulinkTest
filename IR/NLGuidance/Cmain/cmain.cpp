...
int main(int argc, char** argv)
{
    ...
    int testcaseId = -1;
    // Add init code
    struct tag_RTM_NLGuidance_T* NLGuidance_M = new struct tag_RTM_NLGuidance_T[1];
    NLGuidance_M[0].errorStatus = new char[1];
    NLGuidance_M[0].dwork = new struct DW_NLGuidance_T[1];
    ...
    BitmapCov::branchBitmapLength = 14;
    BitmapCov::condBitmapLength = 16;
    BitmapCov::decBitmapLength = 14;
    BitmapCov::mcdcBitmapLength = 7;
    mcdcMeta = new unsigned char[BitmapCov::mcdcBitmapLength] { 1, 1, 1, 1, 1, 1, 1, };
    int munitCov = 0;
    int branchCov = 0;
    int condCov = 0;
    int decCov = 0;
    ...
    while (!shouldReturn)
    {
        ...
        memcpy(&NLGuidance_U_Xtarg[0][0], buffer + 0, 8);
        memcpy(&NLGuidance_U_Xtarg[0][1], buffer + 8, 8);
        memcpy(&NLGuidance_U_Xtarg[0][2], buffer + 16, 8);
        memcpy(&NLGuidance_U_Xv[0][0], buffer + 24, 8);
        memcpy(&NLGuidance_U_Xv[0][1], buffer + 32, 8);
        memcpy(&NLGuidance_U_Xv[0][2], buffer + 40, 8);
        memcpy(&NLGuidance_U_Vv[0][0], buffer + 48, 8);
        memcpy(&NLGuidance_U_Vv[0][1], buffer + 56, 8);
        memcpy(&NLGuidance_U_Vv[0][2], buffer + 64, 8);
        memcpy(&NLGuidance_U_r[0], buffer + 72, 8);
        memcpy(&NLGuidance_Y_yout[0][0], buffer + 80, 8);
        memcpy(&NLGuidance_Y_yout[0][1], buffer + 88, 8);
        memcpy(&NLGuidance_Y_yout[0][2], buffer + 96, 8);
